RSpec.describe Ec2spec::PriceCalculator do
  describe '.prepare' do
    it 'accepts api calc type' do
      expect(Ec2spec::PriceCalculator.instance.prepare('JPY', 110, 'api'))
  ...
  describe '.currency_values?' do
    ...
    context 'having both @currency_unit and @dollar_exchange_rate' do
      before { Ec2spec::PriceCalculator.instance.prepare('JPY', 110, 'api') }
      it { is_expected.to be_truthy }
    end
    context 'having only @currency_unit' do
      before { Ec2spec::PriceCalculator.instance.prepare('JPY', nil, 'api') }
      it { is_expected.to be_falsey }
    end
    context 'having only @dollar_exchange_rate' do
      before { Ec2spec::PriceCalculator.instance.prepare(nil, 110, 'api') }
      it { is_expected.to be_falsey }
    end
    context 'having neither @currency_unit nor @dollar_exchange_rate' do
      before { Ec2spec::PriceCalculator.instance.prepare(nil, nil, 'api') }
      it { is_expected.to be_falsey }
    end
  end
end
